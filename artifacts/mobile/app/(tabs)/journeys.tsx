...
type SortKey =
  "date_desc" | "date_asc" | "distance" | "duration" | "top_speed" | "xp";
...
const CATEGORY_ICONS: Record<string, string> = {
  "cat-1": "briefcase-outline",
  "cat-2": "leaf-outline",
  "cat-3": "map-outline",
  "cat-4": "people-outline",
  "cat-5": "camera-outline",
  "cat-6": "flag-outline",
  "cat-7": "sunny-outline",
};
...
export default function JourneysScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterType === "personal")
      list = list.filter((j) => j.journeyType !== "convoy");
    if (filterType === "convoy")
      list = list.filter((j) => j.journeyType === "convoy");
    if (filterCategoryId)
      list = list.filter((j) => j.categoryId === filterCategoryId);
  ...
  const stats = useMemo(() => {
    ...
    const avgSpeed =
      list.length > 0
        ? Math.round(list.reduce((s, j) => s + j.averageSpeed, 0) / list.length)
        : 0;
    ...
    list.forEach((j) => {
      vehicleCounts[j.vehicleId] = (vehicleCounts[j.vehicleId] ?? 0) + 1;
    });
    const topVehicleId = Object.entries(vehicleCounts).sort(
      (a, b) => b[1] - a[1],
    )[0]?.[0];
    const topVehicle = topVehicleId
      ? vehicles.find((v) => v.id === topVehicleId)
      : null;
  ...
  function JourneyCard({ item }: { item: Journey }) {
    ...
    return (
      ...
      >
        <RouteTrace coordinates={item.routeCoordinates ?? []} />
        ...
        <View style={styles.cardMeta}>
          ...
          {cat && (
            <View
              style={[
                styles.tag,
                { borderLeftWidth: 2, borderLeftColor: cat.colour },
              ]}
            >
              <Ionicons name={cat.icon as any} size={10} color={cat.colour} />
              <Text style={[styles.tagText, { color: cat.colour }]}>
                {cat.name}
              </Text>
            </View>
          )}
        ...
        <View style={styles.statsGrid2}>
          <View style={styles.statItem}>
            <Text style={styles.statItemValue}>
              {formatDistance(item.distance, resolvedUnitSystem)}
            </Text>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statItemValue}>
              {formatSpeed(item.averageSpeed, resolvedUnitSystem)}
            </Text>
            <Text style={styles.statItemLabel}>Avg</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={[styles.statItemValue, { color: colors.primary }]}>
              {formatSpeed(item.topSpeed, resolvedUnitSystem)}
            </Text>
  ...
  const currentSortLabel =
    SORT_OPTIONS.find((s) => s.key === sortBy)?.label ?? "Recent";
  const activeCatFilter = filterCategoryId
              ...
              <View style={styles.primaryStats}>
                ...
                <View style={styles.primaryCard}>
                  <Text
                    ...
                    style={[styles.primaryValue, { color: colors.primary }]}
                  >
                    {formatDistance(stats.totalKm, resolvedUnitSystem)}
                  </Text>
                  <Text style={styles.primaryLabel}>Total Distance</Text>
                </View>
              </View>
              <Disclosure title="Driving statistics">
                {/* XP summary */}
                {stats.totalXP > 0 && (
                  <View style={styles.xpCard}>
                    ...
                    <View>
                      ...
                      <Text style={styles.xpLabel}>
                        earned {PERIOD_LABELS[period].toLowerCase()}
                      </Text>
                    </View>
                  </View>
                )}
                <View style={styles.secondaryStats}>
                  ...
                  <View style={styles.secondaryCard}>
                    <Text style={styles.secondaryValue}>
                      {formatDistance(
                        stats.total > 0 ? stats.totalKm / stats.total : 0,
                        resolvedUnitSystem,
                      )}
                    </Text>
                  ...
                  <View style={styles.secondaryCard}>
                    <Text style={styles.secondaryValue}>
                      {formatSpeed(stats.avgSpeed, resolvedUnitSystem)}
                    </Text>
                    <Text style={styles.secondaryLabel}>Avg Speed</Text>
                  </View>
                  <View style={styles.secondaryCard}>
                    <Text
                      style={[styles.secondaryValue, { color: colors.primary }]}
                    >
                      {formatSpeed(stats.topSpeed, resolvedUnitSystem)}
                    </Text>
                    <Text style={styles.secondaryLabel}>Top Speed</Text>
                  </View>
                  <View style={styles.secondaryCard}>
                    <Text style={styles.secondaryValue}>
                      {formatDistance(stats.longest, resolvedUnitSystem)}
                    </Text>
                    <Text style={styles.secondaryLabel}>Longest</Text>
                  </View>
                  <View style={styles.secondaryCard}>
                    <Text style={styles.secondaryValue}>
                      {stats.personalCount}/{stats.convoyCount}
                    </Text>
                ...
                {stats.topVehicle && (
                  ...
                  >
                    ...
                    />
                    <View>
                      <Text style={[styles.primaryValue, { fontSize: 16 }]}>
                        {stats.topVehicle.nickname}
                      </Text>
            ...
            >
              ...
              {categories.map((cat) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.filterBtn,
                    filterCategoryId === cat.id && styles.filterBtnActive,
                  ]}
                  onPress={() =>
                    setFilterCategoryId((p) => (p === cat.id ? null : cat.id))
                  }
                >
                  <Text
                    style={[
                      styles.filterBtnText,
                      filterCategoryId === cat.id && styles.filterBtnTextActive,
                    ]}
                  >
                    {cat.name}
                  </Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {period !== "all"
                ? `No drives recorded for ${PERIOD_LABELS[period].toLowerCase()}.`
                : "Head to the map and start a drive."}
            </Text>
          </View>
        }
      />
            ...
            {SORT_OPTIONS.map((opt) => (
              ...
              >
                <Text
                  style={[
                    styles.modalRowText,
                    sortBy === opt.key && styles.modalRowActive,
                  ]}
                >
                  {opt.label}
                </Text>
                ...
                <View key={cat.id} style={styles.modalRow}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      Alert.alert(
                        "Delete Category",
                        `Delete "${cat.name}"? Journeys in this category will become uncategorised.`,
                        [
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.prompt
                    ...
                    : Alert.alert(
                        "New Category",
                        "Use the iOS app to create custom categories with icons and colours.",
                      );
                }}
              >
                <Ionicons
